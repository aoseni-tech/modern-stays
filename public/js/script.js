...
if (listing_form) {
    class Validator {
        constructor(value = '', pattern, name) {
            this.value = value;
        ...
        get errorMessage() {
            if (this.value === '')
                return `${this.name.charAt(0).toUpperCase() + this.name.slice(1)} is required`;
            else
                return `Please provide a valid ${this.name}`;
        }
        get isValid() {
            if (this.value === '')
                return false;
    ...
    for (let i = 0; i < form_inputs.length; i++) {
        inputObjectArray.push(new Validator(form_inputs[i].value, pattern[i], form_inputs[i].getAttribute('name')));
    }
