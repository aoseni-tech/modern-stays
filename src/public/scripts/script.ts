...
const stay_search_form = document.querySelector(
...
const openModalRadio = document.querySelector(
  '#show-radio'
)! as HTMLInputElement;
...
const searchPage = document.querySelector('.staysList')! as HTMLElement;
const filter_container = document.querySelector(
  '.staysList .filters'
)! as HTMLElement;
...
function set_stayTop() {
  if (filter_container) {
    ...
    let filter_height =
      filter_container.clientHeight -
      parseFloat(filter_styles.paddingTop) -
      parseFloat(filter_styles.paddingBottom);

    if (window.innerWidth > 950) {
      let stayTop = navBar.clientHeight;
      let searchPage_top =
        navBar.clientHeight + searchForm.clientHeight + filter_height;
      if (stay_search_form) {
...
doc.addEventListener('click', function (e) {
  ...
  if (openModal && openModalRadio.checked) {
    ...
    this.classList.remove('overflow-hide');
  }
});
  ...
  for (let i = 0; i < form_inputs.length; i++) {
    ...
    if (!form_inputs[i].checkValidity()) {
      if (form_inputs[i].value === '')
        feedback[i].innerHTML = `${form_inputs[i].name} is required`;
...
//filter radio logics
const sort_radios = document.querySelectorAll(
  '.sort-by input[type=radio]'
)! as NodeListOf<HTMLInputElement>;
const sort_options = document.querySelectorAll(
  '.sort-by label'
)! as NodeListOf<HTMLInputElement>;
const sort_menu = document.querySelector('.sort-by ul')! as HTMLElement;
const sort_input = document.querySelector(
  '.sort-by input[type=hidden]'
)! as HTMLInputElement;
const sort_input_text = document.querySelector(
  '.sort-by input[type=text]'
)! as HTMLInputElement;

const filter_radios = document.querySelectorAll(
...
const filter_input_text = document.querySelector(
  '.filter input[type=text]'
)! as HTMLInputElement;

const filtersInput = document.querySelectorAll(
  '.filters input[type=hidden]'
)! as NodeListOf<HTMLInputElement>;

function dropDrown(
  options: NodeListOf<HTMLInputElement>,
  readInput: HTMLInputElement,
  menu: HTMLElement,
  ...
  if (textInput) {
    ...
    window.addEventListener('click', (e) => {
      if (
        !textInput.contains(e.target as HTMLElement) &&
...
let actDeleteForm = document.querySelector(
  '#actDelete-form'
)! as HTMLFormElement;
let actDeleteBtn = document.querySelector(
  '#actDelete-btn'
...
if (stars) {
  stars.forEach((star: HTMLSpanElement) => {
    ...
    if (
      star.dataset.rating &&
      typeof parseFloat(star.dataset.rating) === 'number'
    ) {
...
let carousel_prevBtn = document.querySelectorAll(
  `.carousel_prevBtn`
)! as NodeListOf<HTMLButtonElement>;
let carousel_nxtBtn = document.querySelectorAll(
  `.carousel_nxtBtn`
)! as NodeListOf<HTMLButtonElement>;
...
if (scenes) {
  for (let i = 0; i < scenes.length; i++) {
    ...
    for (let i = 0; i < cells.length; i++) {
      cells[i].style.transform = `rotateY(${
        i * carouselAngle
...
let toggleMap =  document.querySelector('#toggleMap')! as HTMLInputElement;
let toggleMapLabel =  document.querySelector('.toggleMap')! as HTMLInputElement;
if(toggleMap) {
  toggleMap.addEventListener('change', function(){
       if(this.checked) {
        toggleMapLabel.innerHTML = `show list ≡`
       } else {
        toggleMapLabel.innerHTML = `show map 🏳`
       }
  })
}